...
router.get("/movie", async (req, res) => {
  try {
    ...
    const reply =
      movies.length > 0 ? { data: movies } : { message: "no results found" };
    res.json(reply);
  } catch (err) {
    console.error(err);
...
router.route("/actors").get(async (req, res) => {
  try {
    ...
    const reply =
      actors.length > 0 ? { data: actors } : { message: "no results found" };
    res.json(reply);
  } catch (err) {
    console.error(err);
    res.json({ message: "Something went wrong" });
...
router.route("/ratings").get(async (req, res) => {
  try {
    ...
    const reply =
      ratings.length > 0 ? { data: ratings } : { message: "no results found" };
    res.json(reply);
  } catch (err) {
    console.error(err);
    res.json({ message: "Something went wrong" });
